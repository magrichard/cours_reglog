...
- L’estimation de  $\widehat{\beta}$  est donc fait **par approximation**.
...
En d'autres mots: quel est l'effet de la variable $X_k$ sur $logit(P(Y=1|X))$
 ...
 $logit(\pi(X)=\beta X)$ <=> $log \Big(\frac{\pi(X)}{1-\pi(X)}\Big) =\beta X$

On defini $Odds(X) = \frac{\pi(X)}{1-\pi(X)}$ <=> $Odds(X) = e^{(\beta X)}$
